...
const DayPassForm = () => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const response = await fetch("/api/daypass", {
                method: "POST",
            ...
            if(data.error) {
                ...
                toast.success("Day pass issued successfully");
                setNumberPlate("");
                setVehicleType("");
            }
        } catch {
  ...
  return (
    <div className="max-w-md mx-auto p-6 bg-white rounded-lg shadow-md">
      <h2 className="text-2xl font-bold mb-6 text-center">Day Pass Entry</h2>
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div className="flex gap-3">
          <Button 
            ...
            disabled={status === 'loading'}
          >
            {status === 'loading' ? 'Processing...' : 'Issue Day Pass'}
          </Button>
          <Button 
            type="button" 
            variant="outline"
